...
int ib_connect_qp(struct ibv_qp *local_qp, const QPInfo *local_qp_info,
                  const QPInfo *remote_qp_info, uint8_t conn_type);

void ib_print_gid(const uint8_t *gid);

void ib_print_wr(struct ibv_send_wr *wr_list);

inline void ib_create_sge(uint64_t local_addr, uint32_t lkey, uint32_t len,
