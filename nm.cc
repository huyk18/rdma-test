...
int UDPNetworkManager::ibv_post_send_wrapper(struct ibv_qp *qp,
                                             struct ibv_send_wr *wr,
                                             struct ibv_send_wr **bad_wr) {
  for (auto cur = wr; cur != NULL; cur = cur->next) {
    if (!(cur->send_flags & IBV_SEND_SIGNALED)) {
      outstanding_unsignaled_send_++;
    }
  }
  if (outstanding_unsignaled_send_ > 256 &&
    ...
    assert(wc.wr_id == wr->wr_id);
    return ret;
  }
  return ibv_post_send(qp, wr, bad_wr);
}

int UDPNetworkManager::ibv_poll_cq_send_wrapper(struct ibv_cq *cq,
                                                int num_entries,
                                                struct ibv_wc *wc) {
  outstanding_unsignaled_send_ = 0;
...
int UDPNetworkManager::UDPNMInitServer(const DMCConfig *conf) {
  ...
  printd(L_INFO, "Server's nm initialized. server_id: %d", conf->server_id);
  return 0;
}
...
int UDPNetworkManager::UDPNMInitClient(const DMCConfig *conf) {
  ...
  printd(L_INFO, "Client's nm initialized. server_id: %d", conf->server_id);
  return 0;
}
...
int UDPNetworkManager::send_udp_msg(UDPMsg *msg, struct sockaddr_in *dest_addr,
...
void UDPNetworkManager::close_udp_sock() { close(udp_sock_); }
...
#endif
    num_polled = ibv_poll_cq_send_wrapper(ib_send_cq_, 1, wc);
  }
  assert(num_polled == 1);
  return 0;
}
...
int UDPNetworkManager::rdma_write_sid_sync(uint16_t server,
  ...
  int ret = ibv_post_send_wrapper(qp, &sr, &bad_wr);
  assert(ret == 0);
...
int UDPNetworkManager::rdma_inl_write_sid_sync(uint16_t server,
  ...
  sr.opcode = IBV_WR_RDMA_WRITE;
  sr.send_flags =
      IBV_SEND_SIGNALED | IBV_SEND_FENCE | IBV_SEND_INLINE; // inline need <236B
  sr.wr.rdma.remote_addr = remote_addr;
  sr.wr.rdma.rkey = rkey;
...
int UDPNetworkManager::rdma_faa_size_sync(uint16_t server, uint64_t remote_addr,
...
int UDPNetworkManager::rdma_cas_sid_sync(uint16_t server, uint64_t remote_addr,
                                         uint32_t rkey, uint64_t local_addr,
                                         uint32_t lkey, uint64_t expect_val,
                                         uint64_t swap_val) {
  struct ibv_send_wr sr;
  struct ibv_send_wr *bad_wr;
  struct ibv_sge send_sge;
...
int UDPNetworkManager::rdma_batch_read_sid_sync(
    uint16_t server, uint64_t *remote_addr, uint32_t rkey, uint64_t *local_addr,
...
int UDPNetworkManager::rdma_read_sid_sync(uint16_t server, uint64_t remote_addr,
                                          uint32_t rkey, uint64_t local_addr,
                                          uint32_t lkey, uint32_t len) {
  struct ibv_send_wr sr;
  struct ibv_send_wr *bad_wr;
